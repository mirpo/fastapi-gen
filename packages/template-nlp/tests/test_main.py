...
def test_ner_post_success(client):
    ...
    # NER is deterministic: "William Henry Gates III" must be recognized as a person
    assert any(entity["entity_group"] == "PER" for entity in entities)
...
def test_question_answering_post_success(client):
    ...
    # Extractive QA is deterministic: the answer comes verbatim from the context
    assert "engineer" in data["answer"].lower()
...
def test_embeddings_success(client):
    ...
    assert data["dimension"] == len(data["embeddings"][0]) > 0
...
def test_sentiment_success(client):
    ...
    # Sentiment on this input is deterministic
...
def test_classify_success(client):
    ...
    assert sum(data["scores"]) == pytest.approx(1.0, abs=0.01)
...
def test_similarity_success(client):
    ...
    # Embeddings are deterministic; these paraphrases must score as clearly similar
